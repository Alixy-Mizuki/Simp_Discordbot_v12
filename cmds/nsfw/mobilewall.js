...
module.exports = class NS_mobilewallCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'mobilewall',
			aliases: [], 
			group: 'nsfw',
			memberName: 'mobilewall', 
			description: 'random NSFW Wallpaper! (Desktop)', 
      examples: ['mobilewall <count | default: 1>'],
      guildOnly: true,
      hidden: true,
      nsfw: true,
      throttling: 
...
//
  ...
  try
  {
    ...
    {
      return message.channel.send(new MessageAttachment(await AKN.nsfw.mobileWallpapers(), "Mobile.jpg"))
    }
    if(Count > 1) 
    {
      let userMSG = message;
