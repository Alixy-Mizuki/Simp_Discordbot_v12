...
module.exports = class NS_FemdomCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'femdom',
			aliases: ['fdm'], 
      ...
      examples: ['femdom <count | default: 1>'],
      guildOnly: true,
      hidden: true,
      nsfw: true,
      throttling: 
...
//
  ...
  try
  {
    ...
    {
      return message.channel.send(new MessageAttachment(await AKN.nsfw.femdom(), "femdom.jpg"))
    }
    if(Count > 1) 
    {
      let userMSG = message;
      ...
      for(let i = 0; i < Count; i++)
      {
        ...
        .setFile(await AKN.nsfw.femdom(), `femdom-${i}.jpg`)
        Images.push(IMG)
      }
