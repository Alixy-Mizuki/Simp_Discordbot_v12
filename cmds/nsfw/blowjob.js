...
module.exports = class NS_blowjobCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'blowjob',
			aliases: ['bjob'], 
      ...
      examples: ['blowjob <count | default: 1>', 'bjob <count | default: 1>'],
      guildOnly: true,
      hidden: true,
      nsfw: true,
      throttling: 
...
//
  ...
  try
  {
    ...
    {
      return message.channel.send(new MessageAttachment(await AKN.nsfw.blowjob(), "blowjob.jpg"))
    }
    if(Count > 1) 
    {
      let userMSG = message;
      ...
      for(let i = 0; i < Count; i++)
      {
        ...
        .setFile(await AKN.nsfw.blowjob(), `blowjob-${i}.jpg`)
        Images.push(IMG)
      }
