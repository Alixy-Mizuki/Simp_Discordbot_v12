...
module.exports = class NS_succubusCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'succubus',
			aliases: [], 
      ...
      examples: ['succubus <count | default: 1>'],
      guildOnly: true,
      hidden: true,
      nsfw: true,
      throttling: 
...
//
  ...
  try
  {
    ...
    {
      return message.channel.send(new MessageAttachment(await AKN.nsfw.succubus(), "succubus.jpg"))
    }
    if(Count > 1) 
    {
      let userMSG = message;
      ...
      for(let i = 0; i < Count; i++)
      {
        ...
        .setFile(await AKN.nsfw.succubus(), `succubus-${i}.jpg`)
        Images.push(IMG)
      }
