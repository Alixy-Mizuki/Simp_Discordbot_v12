...
module.exports = class NS_FoxgirlCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'foxgirl',
			aliases: [], 
      ...
      examples: ['foxgirl <count | default: 1>'],
      guildOnly: true,
      hidden: true,
      nsfw: true,
      throttling: 
...
//
  ...
  try
  {
    ...
    {
      return message.channel.send(new MessageAttachment(await AKN.nsfw.foxgirl(), "foxgirl.jpg"))
    }
    if(Count > 1) 
    {
      let userMSG = message;
      ...
      for(let i = 0; i < Count; i++)
      {
        ...
        .setFile(await AKN.nsfw.foxgirl(), `foxgirl-${i}.jpg`)
        Images.push(IMG)
      }
