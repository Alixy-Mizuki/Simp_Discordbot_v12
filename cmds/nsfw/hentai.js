...
module.exports = class NS_hentaiCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'hentai',
			aliases: [], 
      ...
      examples: ['hentai <count | default: 1>'],
      guildOnly: true,
      hidden: true,
      nsfw: true,
      throttling: 
...
//
  ...
  try
  {
    ...
    {
      return message.channel.send(new MessageAttachment(await AKN.nsfw.hentai(), "hentai.jpg"))
    }
    if(Count > 1) 
    {
      let userMSG = message;
      ...
      for(let i = 0; i < Count; i++)
      {
        ...
        .setFile(await AKN.nsfw.hentai(), `hentai-${i}.jpg`)
        Images.push(IMG)
      }
