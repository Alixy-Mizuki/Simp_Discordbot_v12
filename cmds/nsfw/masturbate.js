...
module.exports = class NS_masturbationCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'masturbate',
			aliases: ['masturbation'], 
			group: 'nsfw',
			memberName: 'masturbate', 
			description: "Solo Queue in CSGO!", 
      examples: ['masturbation <count | default: 1>'],
      guildOnly: true,
      hidden: true,
      nsfw: true,
      throttling: 
...
//
  ...
  try
  {
    ...
    {
      return message.channel.send(new MessageAttachment(await AKN.nsfw.masturbation(), "masturbation.jpg"))
    }
    if(Count > 1) 
    {
      let userMSG = message;
