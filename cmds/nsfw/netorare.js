...
module.exports = class NS_netorareCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'netorare',
			aliases: [], 
      ...
      examples: ['netorare <count | default: 1>'],
      guildOnly: true,
      hidden: true,
      nsfw: true,
      throttling: 
...
//
  ...
  try
  {
    ...
    {
      return message.channel.send(new MessageAttachment(await AKN.nsfw.netorare(), "netorare.jpg"))
    }
    if(Count > 1) 
    {
      let userMSG = message;
      ...
      for(let i = 0; i < Count; i++)
      {
        ...
        .setFile(await AKN.nsfw.netorare(), `netorare-${i}.jpg`)
        Images.push(IMG)
      }
