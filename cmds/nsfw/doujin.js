...
module.exports = class NS_doujinCommand extends Command { //MeowCommand
	constructor(client) {
		super(client, {
			name: 'doujin',
			aliases: [], 
      ...
      examples: ['doujin <count | default: 1>'],
      guildOnly: true,
      hidden: true,
      nsfw: true,
      throttling: 
...
//
  ...
  try
  {
    ...
    {
      return message.channel.send(new MessageAttachment(await AKN.nsfw.doujin(), "doujin.jpg"))
    }
    if(Count > 1) 
    {
      let userMSG = message;
      ...
      for(let i = 0; i < Count; i++)
      {
        ...
        .setFile(await AKN.nsfw.doujin(), `doujin-${i}.jpg`)
        Images.push(IMG)
      }
